...
$harian = $conn->query("
    SELECT DATE(t.created_at) as tgl,
           ...
           SUM(dt.jumlah*p.harga_beli) as modal
    FROM transaksi t
    LEFT JOIN detail_transaksi dt ON t.id=dt.transaksi_id
    LEFT JOIN produk p ON dt.produk_id=p.id
    WHERE DATE(t.created_at) BETWEEN '$tgl1' AND '$tgl2' AND t.status_transaksi!='kredit'
    GROUP BY DATE(t.created_at) ORDER BY tgl DESC
");
...
$terlaris = $conn->query("
    ...
    GROUP BY dt.produk_id ORDER BY terjual DESC LIMIT 10
");
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;flex-wrap:wrap;gap:14px;">
  <div>
    ...
    <p style="font-size:.84rem;color:#b08fa0;">Data penjualan, modal & keuntungan GYA Cosmetics</p>
  </div>
</div>
...
<div class="row g-3 mb-4">
  <div class="col-xl-3 col-md-6">
    <div class="stat-card" style="border-top:3px solid rgba(139,92,246,.5);">
      <div class="stat-icon-wrap" style="background:rgba(139,92,246,.15);"><i class="bi bi-cash-coin" style="color:#7c3aed;font-size:1.3rem;"></i></div>
  ...
  <div class="col-xl-3 col-md-6">
    <div class="stat-card" style="border-top:3px solid rgba(59,130,246,.5);">
  ...
  <div class="col-xl-3 col-md-6">
    <div class="stat-card" style="border-top:3px solid rgba(16,185,129,.5);">
      ...
      <div class="stat-change" style="color:#10b981;">
        <?= $total_pend > 0 ? round($keuntungan/$total_pend*100,1).'%' : '0%' ?> margin
      </div>
    </div>
  </div>
  <div class="col-xl-3 col-md-6">
  ...
  <div class="col-lg-7">
    <div class="content-card">
      <div class="content-card-header">
        <div class="content-card-title"><i class="bi bi-bar-chart-line" style="color:#7c3aed;"></i> Grafik 7 Hari Terakhir</div>
      </div>
  ...
  <div class="col-lg-5">
    <div class="content-card" style="height:100%;">
      ...
      <div style="max-height:288px;overflow-y:auto;">
        ...
        <div style="padding:12px 18px;display:flex;align-items:center;gap:11px;border-bottom:1px solid rgba(139,92,246,.06);">
          <span style="font-size:1.1rem;flex-shrink:0;"><?=$rank<=3?$medal[$rank]:"<span style='font-size:.78rem;color:#b08fa0;font-weight:700;'>$rank</span>"?></span>
          <div style="flex:1;min-width:0;">
            ...
            <div style="font-size:.72rem;color:#b08fa0;"><?=htmlspecialchars($p['brand']??'')?> · <?=formatRupiah($p['omzet'])?></div>
          </div>
...
<div class="row g-3 mb-4">
  <div class="col-12">
    <div class="content-card">
      <div class="content-card-header"><div class="content-card-title"><i class="bi bi-tags" style="color:#7c3aed;"></i> Penjualan per Kategori</div></div>
      <div style="overflow-x:auto;">
        <table class="table-gya">
          ...
          <tbody>
            ...
            ?>
            <tr>
              <td><span class="badge-gya badge-purple"><?=htmlspecialchars($r['nama_kategori'])?></span></td>
              ...
              <td>
                <div style="display:flex;align-items:center;gap:8px;">
                  <div style="flex:1;height:6px;background:rgba(139,92,246,.1);border-radius:3px;overflow:hidden;">
                    <div style="height:100%;width:<?=$pct?>%;background:linear-gradient(90deg,#7c3aed,#a78bfa);border-radius:3px;"></div>
                  </div>
                  <span style="font-size:.78rem;font-weight:600;color:#7c3aed;white-space:nowrap;"><?=$pct?>%</span>
                </div>
              </td>
            </tr>
            <?php endwhile; else: ?>
...
<!-- TABEL HARIAN -->
<div class="content-card">
  <div class="content-card-header"><div class="content-card-title"><i class="bi bi-table" style="color:#7c3aed;"></i> Detail Harian</div></div>
  <div style="overflow-x:auto;">
    <table class="table-gya">
...
(function(){
  ...
  grad.addColorStop(0,'rgba(139,92,246,.28)');
  grad.addColorStop(1,'rgba(139,92,246,.01)');
  new Chart(ctx,{type:'bar',data:{labels:".json_encode($g_label).",datasets:[{label:'Penjualan',data:".json_encode($g_pend).",backgroundColor:grad,borderColor:'#7c3aed',borderWidth:2,borderRadius:8,borderSkipped:false}]},options:{responsive:true,plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(26,5,18,.88)',titleColor:'#c4b5fd',bodyColor:'#fff',padding:12,cornerRadius:10,callbacks:{label:function(c){return' Rp '+c.raw.toLocaleString('id-ID');}}}},scales:{y:{beginAtZero:true,grid:{color:'rgba(139,92,246,.07)'},border:{display:false},ticks:{color:'#b08fa0',font:{size:11},callback:function(v){if(v>=1e6)return'Rp '+(v/1e6).toFixed(1)+'jt';if(v>=1e3)return'Rp '+(v/1e3).toFixed(0)+'rb';return'Rp '+v;}}},x:{grid:{display:false},border:{display:false},ticks:{color:'#b08fa0',font:{size:11}}}}}});
})();
</script>";?>
