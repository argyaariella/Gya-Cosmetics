...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $aksi = $_POST['aksi'] ?? '';
    if ($aksi === 'tambah') {
        $judul = bersihkan($_POST['judul']); $desk = bersihkan($_POST['deskripsi']??'');
        $dis = (int)$_POST['diskon_persen']; $tm = bersihkan($_POST['tanggal_mulai']??'');
        $ts = bersihkan($_POST['tanggal_selesai']??''); $sts = bersihkan($_POST['status']);
        if (empty($judul)) { setAlert('Judul promo wajib!','danger'); }
        else {
            ...
            if($st->execute()) setAlert("Promo <b>$judul</b> ditambahkan!",'success'); else setAlert('Gagal!','danger');
            $st->close();
        }
    } elseif ($aksi === 'edit') {
        $id=(int)$_POST['id']; $judul=bersihkan($_POST['judul']); $desk=bersihkan($_POST['deskripsi']??'');
        $dis=(int)$_POST['diskon_persen']; $tm=bersihkan($_POST['tanggal_mulai']??'');
        $ts=bersihkan($_POST['tanggal_selesai']??''); $sts=bersihkan($_POST['status']);
        ...
        if($st->execute()) setAlert('Promo diperbarui!','success'); else setAlert('Gagal!','danger');
        $st->close();
    } elseif ($aksi === 'hapus') {
        $id=(int)$_POST['id']; $conn->query("DELETE FROM promo WHERE id=$id");
        setAlert('Promo dihapus!','success');
    } elseif ($aksi === 'toggle') {
        $id=(int)$_POST['id']; $sts=bersihkan($_POST['current_status'])==='aktif'?'nonaktif':'aktif';
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;flex-wrap:wrap;gap:14px;">
  <div>
    ...
    <p style="font-size:.84rem;color:#b08fa0;">
      <?= $promo_aktif ?> promo aktif saat ini · Kelola diskon & promo toko
    </p>
  </div>
    ...
    <div class="stat-card" style="border-top:3px solid <?=$aktif_skrg?'rgba(16,185,129,.5)':($kadaluarsa?'rgba(100,100,100,.2)':'rgba(245,158,11,.4)')?>;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px;">
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <?php if($aktif_skrg): ?><span class="badge-gya badge-success"><i class="bi bi-circle-fill" style="font-size:.45rem;"></i> Aktif</span>
          <?php elseif($kadaluarsa): ?><span class="badge-gya badge-danger">Kadaluarsa</span>
          <?php else: ?><span class="badge-gya badge-warning">Nonaktif</span><?php endif; ?>
        </div>
        <div style="display:flex;gap:5px;">
          <form method="POST" style="display:inline;">
            <input type="hidden" name="aksi" value="toggle"><input type="hidden" name="id" value="<?=$p['id']?>"><input type="hidden" name="current_status" value="<?=$p['status']?>">
            <button type="submit" class="btn-gya btn-glass-gya btn-sm-gya" title="<?=$p['status']==='aktif'?'Nonaktifkan':'Aktifkan'?>"><i class="bi bi-<?=$p['status']==='aktif'?'pause-circle':'play-circle'?>"></i></button>
          </form>
          <button class="btn-gya btn-glass-gya btn-sm-gya" onclick='editPromo(<?=json_encode($p)?>'><i class="bi bi-pencil"></i></button>
      ...
      <div style="font-family:'Cormorant Garamond',serif;font-size:1.2rem;font-weight:600;color:#1a0a14;margin-bottom:6px;line-height:1.3;"><?=htmlspecialchars($p['judul'])?></div>
      ...
      <div style="font-family:'Cormorant Garamond',serif;font-size:2.8rem;font-weight:700;color:#f43f88;line-height:1;margin-bottom:6px;">
        <?=$p['diskon_persen']?>% <span style="font-size:1rem;font-weight:400;color:#b08fa0;">DISKON</span>
      </div>
      <?php endif; ?>
      <?php if($p['deskripsi']): ?><p style="font-size:.81rem;color:#7c3f5e;margin-bottom:10px;line-height:1.55;"><?=htmlspecialchars($p['deskripsi'])?></p><?php endif; ?>
      <?php if($p['tanggal_mulai']||$p['tanggal_selesai']): ?>
      <div style="background:rgba(244,63,136,.05);border:1px solid rgba(244,63,136,.1);border-radius:10px;padding:8px 12px;font-size:.74rem;color:#9d5a78;display:flex;align-items:center;gap:6px;">
        <i class="bi bi-calendar-range"></i>
        <?=$p['tanggal_mulai']?formatTanggal($p['tanggal_mulai']):'—'?> → <?=$p['tanggal_selesai']?formatTanggal($p['tanggal_selesai']):'—'?>
      </div>
      <?php endif; ?>
    </div>
  </div>
...
<div class="content-card">
  <div class="empty-state" style="padding:70px 20px;">
    ...
    <button onclick="openModal('mTambah')" class="btn-gya btn-primary-gya"><i class="bi bi-plus-circle"></i> Buat Promo Pertama</button>
  </div>
</div>
<?php endif; ?>
...
<div class="modal-gya" id="mTambah"><div class="modal-overlay"></div>
  <div class="modal-box">
    <div class="modal-header-gya"><div class="modal-title-gya"><i class="bi bi-megaphone" style="color:#f43f88;margin-right:8px;"></i>Buat Promo Baru</div><button class="modal-close" onclick="closeModal('mTambah')"><i class="bi bi-x-lg"></i></button></div>
...
<div class="modal-gya" id="mEdit"><div class="modal-overlay"></div>
  <div class="modal-box">
    <div class="modal-header-gya"><div class="modal-title-gya"><i class="bi bi-pencil-square" style="color:#3b82f6;margin-right:8px;"></i>Edit Promo</div><button class="modal-close" onclick="closeModal('mEdit')"><i class="bi bi-x-lg"></i></button></div>
    <form method="POST"><input type="hidden" name="aksi" value="edit"><input type="hidden" name="id" id="eId">
      <div class="modal-body-gya">
        <div class="form-group"><label class="form-label-gya">Judul</label><input type="text" name="judul" id="eJudul" class="form-control-gya" required></div>
...
<script>
function editPromo(d){document.getElementById('eId').value=d.id;document.getElementById('eJudul').value=d.judul;document.getElementById('eDesk').value=d.deskripsi||'';document.getElementById('eDis').value=d.diskon_persen||0;document.getElementById('eTm').value=d.tanggal_mulai||'';document.getElementById('eTs').value=d.tanggal_selesai||'';document.getElementById('eSts').value=d.status;openModal('mEdit');}
</script>
