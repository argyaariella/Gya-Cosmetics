...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($aksi==='tambah') {
        ...
        else{$st=$conn->prepare("INSERT INTO supplier(nama,no_hp,alamat)VALUES(?,?,?)");$st->bind_param("sss",$nama,$hp,$al);if($st->execute()){simpanLog($_SESSION['user_id'],'tambah_supplier',"Tambah: $nama");setAlert("Supplier <b>$nama</b> ditambahkan!",'success');}else setAlert('Gagal!','danger');$st->close();}
    } elseif ($aksi==='edit') {
        $id=(int)$_POST['id'];$nama=bersihkan($_POST['nama']);$hp=bersihkan($_POST['no_hp']??'');$al=bersihkan($_POST['alamat']??'');
        $st=$conn->prepare("UPDATE supplier SET nama=?,no_hp=?,alamat=? WHERE id=?");$st->bind_param("sssi",$nama,$hp,$al,$id);if($st->execute())setAlert('Supplier diperbarui!','success');else setAlert('Gagal!','danger');$st->close();
    } elseif ($aksi==='hapus') {
        $id=(int)$_POST['id'];
...
$rows=$conn->query("SELECT s.*,(SELECT COUNT(*) FROM barang_masuk b WHERE b.supplier_id=s.id) as jml FROM supplier s ORDER BY s.nama");
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:28px;flex-wrap:wrap;gap:14px;">
  ...
  <button onclick="openModal('mTambah')" class="btn-gya btn-primary-gya"><i class="bi bi-plus-circle"></i> Tambah Supplier</button>
</div>

<div class="content-card">
  <div style="overflow-x:auto;">
    <table class="table-gya" style="min-width:600px;">
      <thead><tr><th>#</th><th>Nama Supplier</th><th>No. HP</th><th>Alamat</th><th>Barang Masuk</th><th style="text-align:center;">Aksi</th></tr></thead>
      <tbody>
        ...
        <tr>
          ...
          <td>
            <div style="display:flex;align-items:center;gap:10px;">
              <div style="width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,rgba(244,63,136,.15),rgba(232,121,249,.15));display:flex;align-items:center;justify-content:center;color:#f43f88;font-size:1rem;flex-shrink:0;">
          ...
          <td>
            <?php if($s['no_hp']):?>
            <a href="https://wa.me/<?=preg_replace('/[^0-9]/','',$s['no_hp'])?>" target="_blank" style="color:#25d366;text-decoration:none;font-size:.86rem;display:flex;align-items:center;gap:5px;">
              <i class="bi bi-whatsapp"></i> <?=htmlspecialchars($s['no_hp'])?>
            </a>
            <?php else:?><span style="color:#b08fa0;font-size:.84rem;">—</span><?php endif;?>
          </td>
          <td style="font-size:.84rem;color:#7c3f5e;max-width:220px;"><?=htmlspecialchars($s['alamat']??'—')?></td>
          <td><span class="badge-gya badge-pink"><?=$s['jml']?> transaksi</span></td>
        ...
        <?php endwhile;else:?>
        <tr><td colspan="6" class="empty-state"><span class="empty-icon"><i class="bi bi-truck"></i></span><div>Belum ada data supplier</div></td></tr>
        <?php endif;?>
      </tbody>
    </table>
  </div>
...
<div class="modal-gya" id="mTambah"><div class="modal-overlay"></div>
  <div class="modal-box">
    <div class="modal-header-gya"><div class="modal-title-gya"><i class="bi bi-truck" style="color:#f43f88;margin-right:8px;"></i>Tambah Supplier</div><button class="modal-close" onclick="closeModal('mTambah')"><i class="bi bi-x-lg"></i></button></div>
...
<div class="modal-gya" id="mEdit"><div class="modal-overlay"></div>
  <div class="modal-box">
    ...
    <form method="POST"><input type="hidden" name="aksi" value="edit"><input type="hidden" name="id" id="eId">
      <div class="modal-body-gya">
        <div class="form-group"><label class="form-label-gya">Nama Supplier</label><input type="text" name="nama" id="eNama" class="form-control-gya" required></div>
...
<script>
function editSup(d){document.getElementById('eId').value=d.id;document.getElementById('eNama').value=d.nama;document.getElementById('eHp').value=d.no_hp||'';document.getElementById('eAl').value=d.alamat||'';openModal('mEdit');}
</script>
